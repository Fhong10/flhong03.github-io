---
layout: archive
title: "Research Projects"
permalink: /research/
author_profile: true
---

Here is a summary of my work in optical simulation and semiconductor physics. Click on any project to see the full validation report and code.

<hr>

<div style="display: flex; flex-direction: row; align-items: flex-start; margin-bottom: 20px;">
  <div style="width: 30%; padding-right: 15px;">
    <a href="/research/project-1/">
      <img src="/images/561nm.png" alt="561nm Graph" style="max-width: 100%; height: auto; border: 1px solid #e0e0e0; box-shadow: 2px 2px 5px rgba(0,0,0,0.1);">
    </a>
  </div>
  <div style="width: 70%; font-size: 15px;">
    <a href="/research/project-1/" style="text-decoration: none;">
      <strong style="font-size: 18px; color: #333;">Algorithm Validation: 561nm Target</strong>
    </a>
    <br>
    <p style="margin-top: 5px;">
      I reproduced the reflectivity stopband for a 561nm target using my custom Python TMM code. This validated the solver against standard textbook results.
    </p>
    <a href="/research/project-1/" style="font-weight: bold;">[Read Full Report]</a>
  </div>
</div>

<hr>

<div style="display: flex; flex-direction: row; align-items: flex-start; margin-bottom: 20px;">
  <div style="width: 30%; padding-right: 15px;">
    <a href="/research/project-2/">
      <img src="/images/Absorbance2.png" alt="Absorbance Graph" style="max-width: 100%; height: auto; border: 1px solid #e0e0e0; box-shadow: 2px 2px 5px rgba(0,0,0,0.1);">
    </a>
  </div>
  <div style="width: 70%; font-size: 15px;">
    <a href="/research/project-2/" style="text-decoration: none;">
      <strong style="font-size: 18px; color: #333;">Optical Absorbance Analysis</strong>
    </a>
    <br>
    <p style="margin-top: 5px;">
      A detailed study of material absorbance properties across the visible spectrum, comparing simulation data with experimental results.
    </p>
    <a href="/research/project-2/" style="font-weight: bold;">[Read Full Report]</a>
  </div>
</div>

<hr>

<div style="display: flex; flex-direction: row; align-items: flex-start; margin-bottom: 20px;">
  <div style="width: 30%; padding-right: 15px;">
    <a href="/research/project-3/">
      <img src="/images/semiconductor.png" alt="Semiconductor Image" style="max-width: 100%; height: auto; border: 1px solid #e0e0e0; box-shadow: 2px 2px 5px rgba(0,0,0,0.1);">
    </a>
  </div>
  <div style="width: 70%; font-size: 15px;">
    <a href="/research/project-3/" style="text-decoration: none;">
      <strong style="font-size: 18px; color: #333;">Semiconductor Device Physics</strong>
    </a>
    <br>
    <p style="margin-top: 5px;">
      Simulation of carrier transport and junction behavior in advanced solar cell structures using multi-physics modeling.
    </p>
    <a href="/research/project-3/" style="font-weight: bold;">[Read Full Report]</a>
  </div>
</div>
