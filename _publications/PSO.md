...
<div class="pso-explainer">
  <p>The plot is a Particle Swarm Optimization (PSO) run on two parameters (n and k). Each white dot is a particle position; the red dot marks the global best candidate found; the yellow star marks the final selected solution. Iteration is 40 to find a best mean squared error (MSE) of 5.896×10⁻⁴.</p>
  <p>Color map: It encodes log10(MSE) of the objective over the (n, k) plane.</p>
  <p>Blue/purple = lower MSE (better fit).</p>
  <p>Yellow = higher MSE (worse fit).</p>
  <p>The color bar spans roughly −3 to −1, so MSE ranges from about 10⁻³ to 10⁻¹ across the plotted region.</p>
</div>
## The result
